...
- iOS 26.2で一覧へ戻るアニメーションを録画し、アバターの背景と枠線が一時的に濃くなる現象を再現。
- 背景と枠線を階層的なShapeStyleの`.primary`から明示的な`Color.primary`へ変更し、不透明度を遷移中も維持する。画像、配置、モーダルの表示方式は維持。
- iPhone 17 Pro MaxのSimulatorで、戻るボタンと左端からのスワイプ、モーダルの開閉を録画して変色の解消を確認。ダークモードでの戻る操作も確認。
- iOSビルド・起動、翻訳チェック、SwiftFormat、SwiftLint、`git diff --check`が成功。描画指定だけの変更のため、自動テストの追加・再実行は行っていない。
